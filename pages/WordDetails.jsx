...
const WordDetails = () => {
  ...
  useEffect(() => {
    fetch(`https://api.dictionaryapi.dev/api/v2/entries/en/${word}`)
      .then((res) => res.json())
      .then(([data]) => {
        // console.log(data.meanings[3].definitions[0].example)
        setWordData({
          word: data.word,
  ...
  if (notFound) {
    return (
      <main
        className={`min-h-[calc(100vh-(61.5px+48px))] ${
          isDark ? "bg-slate-700" : "bg-white"
        }`}
      >
        <p
          className={`text-red-500 fixed top-[50%] left-[50%] translate-y-[-50%] translate-x-[-50%] text-3xl font-bold `}
        >
          ...
          <section className="details px-6 sm:px-10 md:px-24 lg:px-40 xl:px-48 mt-6">
            ...
            <div className="definitions">
              ...
              <ol className="list-decimal list-inside mt-2">
                {wordData.definitions.map((meaning) =>
                  meaning.definitions
                    .map((definition) => {
                      return <li key={definition}>{definition.definition}</li>;
                    })
                    .slice(0, 2)
                )}
              </ol>
